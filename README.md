...
The bar pauses briefly after focusing the composer and before selecting a menu entry, and allows up to three seconds for a picker to appear. These pauses are being tested against intermittent opening failures; they are not yet a verified fix. To switch, the bar sends Codex's Control+Shift+M shortcut and checks which picker opens. For the dropdown, it opens Select model and presses the exact model entry without typing into the draft. For the inline `/model` menu, it chooses a recent model with the menu's number shortcut. For another model, it searches by model id, checks that the menu found exactly that model, then presses Return to choose the result. It confirms the changed model in Codex's composer. It stops if you are typing and only removes search text it can prove it added, so it does not deliberately replace or send your draft.

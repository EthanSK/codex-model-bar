...
**Models, one click below Codex.** A 30-point macOS strip follows the active Codex desktop window. Its width fits the model buttons you choose to show, and it highlights the model used by the open task.
...
Click a model in the strip to change the **open Codex task**. Right-click the strip to show or hide model buttons, refresh the list, turn **Open at login** on or off, or quit. Open at login is enabled on the first installed launch; you can disable it from the same menu. Hidden buttons are your local choice and do not remove models from Codex itself.
...
- Accessibility access allows the app to inspect Codex's window and composer. The switcher reads the draft before and after a search to preserve it. It does not store chat text or include telemetry. It stores your hidden-button choices in macOS preferences and caches model names locally in Application Support.

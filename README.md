...
The switcher uses the earlier inline-menu implementation, with a quarter-second pause after focusing and after the menu opens. Compatibility with the current desktop still needs a live switch check. To switch, the bar opens Codex's own `/model` menu with Control+Command+M. It always searches by model id, checks that the menu found exactly that model, then presses Return to choose the result. It confirms the changed model in Codex's composer. It stops if you are typing and only removes search text it can prove it added, so it does not deliberately replace or send your draft.
